...
export function RecentlyAdded() {
  ...
  const { data: albumsData, isLoading: albumsLoading } = useQuery({
    ...
    queryFn: () =>
      api.get<PaginatedResponse<AlbumWithRelations>>('/albums?sort=newest&pageSize=20'),
    enabled: view === 'albums',
  });
  ...
  const { data: tracksData, isLoading: tracksLoading } = useQuery({
    ...
    queryFn: () =>
      api.get<PaginatedResponse<TrackWithRelations>>('/tracks?sort=newest&pageSize=50'),
    enabled: view === 'tracks',
  });
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          {albumsLoading
            ...
            : albumsData?.data.map((album: AlbumWithRelations) => (
                <div
                  key={album.id}
                  className="group relative bg-white/5 p-4 rounded-2xl hover:bg-white/10 transition-all duration-300"
                >
                  <div className="aspect-square rounded-lg overflow-hidden shadow-lg mb-4 relative">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <Button
                        variant="secondary"
                        size="icon"
                        className="rounded-full h-12 w-12 bg-white text-black shadow-xl"
                      >
