    ...
    <div className="flex items-center gap-2">
      ...
      <div className="relative w-24 h-4 flex items-center">
        <div className="absolute left-0 right-0 h-1 bg-white/30 rounded overflow-hidden pointer-events-none">
          <div className="h-full bg-red-600" style={{ width: `${isMuted ? 0 : volume * 100}%` }} />
        </div>
        <input
          type="range"
          min="0"
...
function WatchlistButton({ artist, title }: { artist: string; title: string }) {
  ...
  const mutation = useMutation({
    mutationFn: () => api.post('/watchlist', { artist, title, source: 'radio' }),
    onSuccess: () => {
      setAdded(true);
    ...
    <div className="relative w-full h-full flex items-center px-4">
      ...
      <div className="flex items-center gap-4 w-[30%] pl-2">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <p className="text-sm font-medium text-white truncate">{getTrackTitle()}</p>
            {isRadio && (
            ...
            {isRadio && currentRadioTrack?.artist && currentRadioTrack?.title && (
              <WatchlistButton artist={currentRadioTrack.artist} title={currentRadioTrack.title} />
            )}
          </div>
          <p className="text-xs text-white/50 truncate">{getArtistName()}</p>
      ...
      <div className="flex items-center justify-center gap-2 flex-1">
        ...
        <button
          ...
          className={`p-2 transition-colors ${
            isShuffled ? 'text-red-600 hover:text-red-500' : 'text-white/70 hover:text-white'
          }`}
        >
...
function LastfmIndicator({ isPlaying }: { isPlaying: boolean }) {
  ...
  const isConnected = lastfmStatus?.data?.connected ?? false;
  const tooltip = isConnected
    ? `Last.fm verbunden als ${lastfmStatus?.data?.username ?? '?'}`
  ...
  return (
    <div className="relative w-full h-full flex items-center px-4">
      ...
      <div className="flex items-center justify-center gap-2 flex-1">
        ...
        {/* Stop */}
        <button onClick={stop} className="p-2 text-white/70 hover:text-white ml-2">
          <Square className="h-4 w-4 fill-current" />
        </button>
      </div>
          ...
          <div className="flex items-center justify-center h-full">
            <p className="text-sm text-white/50 italic">
              {section === 'music'
                ? 'Wähle einen Song'
                : section === 'video'
                  ? 'Wähle einen Film oder eine Serie'
                  : 'Wähle ein Hörbuch'}
            </p>
          </div>
        )}
